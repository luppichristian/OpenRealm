...
float GetFaceLightFactor(int normalX, int normalY, int normalZ)
{
  ...
  for (int row = 0; row < cellsHigh; row++)
  {
    for (int column = 0; column < cellsWide; column++)
    {
      ...
      while (column + width < cellsWide)
      {
        ...
        if (!next.valid || next.normalSign != cell.normalSign || !ColorsEqual(next.color, cell.color)) break;
        width += 1;
      }
      ...
      while (row + height < cellsHigh && canGrow)
      {
        for (int widthOffset = 0; widthOffset < width; widthOffset++)
        {
          ...
          if (!next.valid || next.normalSign != cell.normalSign || !ColorsEqual(next.color, cell.color))
          {
            canGrow = false;
            break;
          }
...
BoundingBox ComputeBounds(const ChunkMeshJob& job)
{
  ...
  return BoundingBox{.min = {minX, minY, minZ}, .max = {maxX, maxY, maxZ}};
}
}  // namespace

ChunkMeshJobResult ChunkMesher::Build(const ChunkMeshJob& job)
{
  ...
  auto getVoxelDisplayColor = [](uint8_t voxelValue) {
