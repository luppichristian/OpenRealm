...
static SimulatorConfig BuildSimulatorConfigFromFiles(const NodeFilesConfig& nodeFiles, const NodeRealmFiles& realmFiles)
{
    ...
    SimulatorRuntimeState* runtimeState)
{
  ...
  const RuntimeRealmState realmState = BuildNodeRuntimeRealmState(blockchain, config.blockchainConfig);
...
int RunSimulatorNode(int argc, char** argv)
{
  ...
  NodeRealmFiles realmFiles = {};
  if (!LoadNodeRealmFiles(realmDirectory, &realmFiles, &loadError))
  {
    std::cout << "OpenRealm simulator node\n";
    std::cout << "- runtime config load: failed\n";
