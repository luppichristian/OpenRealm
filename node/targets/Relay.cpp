...
static int RunRelaySmoke(const RelayConfig& config)
{
  ...
  const bool runtimeValidationOk = relayStarted
      ...
      && initialAcceptedPeerDiscovery.received
      && initialAcceptedPeerDiscovery.parsed
      && initialAcceptedPeerDiscovery.decoded
      && initialAcceptedPeerDiscovery.peerDiscovery.nodes.empty()
      && sentDiscoveredPacket
      && discoveredPacket.received
      && discoveredPacket.validation.accepted
      && discoveredPeerDiscovery.received
      && discoveredPeerDiscovery.parsed
      && discoveredPeerDiscovery.decoded
      && discoveredPeerDiscovery.peerDiscovery.nodes.size() == 1
      && sentAcceptedRefreshPacket
      && acceptedRefreshPacket.received
      && acceptedRefreshPacket.validation.accepted
      && acceptedPeerDiscovery.received
      && acceptedPeerDiscovery.parsed
      && acceptedPeerDiscovery.decoded
      && acceptedPeerDiscovery.peerDiscovery.nodes.size() == 1
      ...
      && sentAcceptedInterest
      && sentDiscoveredInterest
      && sentWorldEvent
      && forwardedWorldEvent.received
      && forwardedWorldEvent.parsed
      && forwardedWorldEvent.decoded
      && forwardedWorldEvent.worldEvent.nodeId == discoveredHandshake.nodeId
      && forwardedWorldEvent.worldEvent.event.type == (uint8_t)WorldEventType::Place
      && forwardedWorldEvent.worldEvent.event.voxelValue == 255
      && !senderEchoWorldEvent.received
      && activeNodes.GetCount() == 2
      && chunkInterests.GetCount() == 2
      && stats.sentDiscoveryPackets == 3
      && stats.acceptedInterestPackets == 2
      && stats.forwardedWorldEvents == 1
      && stats.forwardedWorldEventRecipients == 1;
