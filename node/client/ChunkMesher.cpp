...
static uint8_t GetSnapshotVoxel(const ChunkMeshJob& job, int voxelX, int voxelY, int voxelZ)
{
...
static bool IsSnapshotVoxelFilled(const ChunkMeshJob& job, int voxelX, int voxelY, int voxelZ)
{
  return GetSnapshotVoxel(job, voxelX, voxelY, voxelZ) != 0;
}

static float ComputeSnapshotFaceAmbientOcclusion(const ChunkMeshJob& job, const FaceSampleQuery& query)
{
...
static bool AppendGreedyQuad(ChunkMeshBuilder* builder, const GreedyQuadParams& params)
{
  Vector3 corners[4] = {};
...
static bool BuildGreedyMeshPlane(ChunkMeshBuilder* builder, const GreedyPlaneParams& params)
{
...
static BoundingBox ComputeBounds(const ChunkMeshJob& job)
{
